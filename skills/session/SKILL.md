...
A session that lives only in the conversation dies with it. Sessions produce four things worth keeping across runs — the debrief, the backlog, the coverage outline, and the regression checks `/harden` drafts from a session's confirmed bugs — so they are written to a small, predictable tree in **the project you are testing** (the current working directory), never in the plugin's own repo:
