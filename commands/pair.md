...
This is the inversion of `/stride-exploratory-testing:explore`. There, the `explorer` agent drives the app and is forbidden to ask the user anything — charter and environment in, findings out. Here the human is the only actor that touches the product, and the whole command is a conversation. **You have no tool that can reach the application** — no `Agent`, no `WebFetch`, no `Bash` beyond `date` and `mkdir` — and that is deliberate: the division of labour is enforced by the allowlist, not only by this prose.
...
```
Skill(skill: "session", args: "target=<TARGET>; mode=pair")
```

```
Skill(skill: "heuristics", args: "target=<TARGET>; mode=pair")
```

```
Skill(skill: "oracles", args: "target=<TARGET>; mode=pair")
```

```
Skill(skill: "bug-advocacy", args: "mode=pair")
```
...
### Step 3: Open the session with the human (one AskUserQuestion round)
...
Then run a single consolidated `AskUserQuestion` round (≤4 questions):
...
- **Never dispatch the `explorer` agent from here, and never "just run one charter" yourself.** Autonomous execution is `/stride-exploratory-testing:explore` — a different command with a different consent conversation. If the human wants it, say so and stop; do not silently become it.
...
- **Drive the application.** This is the whole distinction from `/stride-exploratory-testing:explore`. There, the `explorer` agent exercises the app and never asks the human anything; here the human exercises the app and *everything* is asked. This command issues no request, opens no browser, runs no CLI against the app, creates no test data, and mutates nothing — and it holds no tool that could: no `Agent`, no `WebFetch`, and no `Bash` beyond `date` and `mkdir`. It may `Read`/`Grep`/`Glob` the source, config, and logs the human named, because reading what the app already produced is observation, not driving.
