...
Close the loop. This plugin's thesis is **Tested = Checked + Explored**, and every other command in it works the Explored half. `/harden` is the bridge back: it takes the bugs a session already found and confirmed with an oracle, and drafts a **regression check** for each one, so a bug that was found once is a bug that stays found.
...
Three things it is not. It does **not** explore — it never touches the running application. It does **not** run anything — it holds no test-runner tool, so it can describe a draft but never report a result for it. And it does **not** edit your test suite — every draft lands in a staging directory, and moving one into your suite is your call and your copy.
...
```
Skill(skill: "bug-advocacy", args: "mode=harden")
```

```
Skill(skill: "session", args: "mode=harden")
```
...
- **Never re-explore to recover a missing repro.** This command does not touch the running application and holds no tool that could. A repro the artifact does not state does not exist for this command.
...
```
DRAFTED BY /stride-exploratory-testing:harden — NOT RUN.
...
**This command has no test-runner tool.** Its `Bash` allowance is `date` and `mkdir`. It has not run any draft, it cannot, and it must say so rather than implying it chose not to.
